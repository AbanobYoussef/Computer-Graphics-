...
namespace PROJECT
{
    public partial class Form1 : Form
    {
        ...
        private void DDA(Point p1,Point p2)
        {
                ...
                dataGridView1.Columns[2].Name = "X-poltted";
                dataGridView1.Columns[3].Name = "Y-poltted";
        ...
        private void sTARTToolStripMenuItem_Click(object sender, EventArgs e)
       {
        ...
        //------------------------------------------------------------------------------------------------
        // rotation about (x,y)---------------------------------------------------------------------------
        private void aROUNDXYToolStripMenuItem_Click(object sender, EventArgs e)
        {
            abxy = true;
            click1 = true;
                ang = double.Parse(Interaction.InputBox("","","enter an angle",-1,-1));
        ...
        private void drawabxy(Point p)
        {
            ...
            for (int i = 0; i < d; i++)
            {
                ...
                switch (s)
                {
                    ...
                    case '5':
                        rot = false;
                        ellipse(cores[i].p1.X, cores[i].p1.Y, cores[i].p2.X);
                        break;
        ...
        private void cLIPINGToolStripMenuItem_Click(object sender, EventArgs e)
        {
            dataGridView1.ColumnCount = 0; 
            pos.Clear();
            if (xmax.Text != "Xmax" && xmin.Text != "Xmin" && ymax.Text != "Ymax" && ymin.Text != "Ymin")
            {

                min.X = Convert.ToInt32(xmin.Text);
                min.Y = Convert.ToInt32(ymin.Text) ;
                max.X = Convert.ToInt32(xmax.Text);
                max.Y = Convert.ToInt32(ymax.Text) ;
                /////////////////////////////////////
                if (max.X > 0 && max.Y > 0)
                {
                    max.Y *= -1;
        ...
        //-----------------------------------------------------------------------------------------------
        //cliping----------------------------------------------------------------------------------------
        private void clip(Point max,Point min)
        {
            ...
            int v;
            pointcode a, b;
            for (int i = 0; i < cores.Count()-4; i++)
            { 
               pos.Add(setcode(cores[i].p1,cores[i].p2 , max, min));
            }
            cores.Clear();
            for (int i = 0; i < pos.Count(); i++)
            {
                v = visibility(pos[i].p1, pos[i].p2);
                pa.X=pos[i].p1.x;
                pa.Y=pos[i].p1.y;
                pb.X = pos[i].p2.x;
                pb.Y = pos[i].p2.y;
                switch (v)
                {
                    ...
                    case 1:
                        break;
                    case 2:
                        a = resetendpt(pos[i].p1, pos[i].p2,  max , min);
                        b = resetendpt(pos[i].p2, pos[i].p1, max, min);
                        pa.X=a.x;
                        pa.Y=a.y;
                        pb.X=b.x;
                        pb.Y = b.y;
                        cores.Add(new core(pa, pb));
                        DDA(pa, pb);
                        break;
                }
            }
